...
```powershell
# AKS クラスターの状態を確認
az aks show -g <RESOURCE_GROUP_NAME> -n <AKS_CLUSTER_NAME> --query "{Name:name, Status:powerState.code, KubernetesVersion:kubernetesVersion, NodeCount:agentPoolProfiles[0].count}" -o table
...
```powershell
# VM の状態を確認
az vm show -g <RESOURCE_GROUP_NAME> -n <MONGODB_VM_NAME> --query "{Name:name, ProvisioningState:provisioningState, PowerState:instanceView.statuses[1].displayStatus}" -o table

# VM のパブリック IP アドレスを取得
az vm list-ip-addresses -g <RESOURCE_GROUP_NAME> -n <MONGODB_VM_NAME> --query "[0].virtualMachine.network.publicIpAddresses[0].ipAddress" -o tsv
```
...
# AKS と ACR を統合
az aks update -g <RESOURCE_GROUP_NAME> -n <AKS_CLUSTER_NAME> --attach-acr <ACR_NAME>
```
...
az monitor log-analytics workspace show -g <RESOURCE_GROUP_NAME> -n <LOG_ANALYTICS_NAME> --query "{Name:name, Location:location}" -o table
```
...
```powershell
# Storage Account を確認
az storage account show -n <STORAGE_ACCOUNT_NAME> -g <RESOURCE_GROUP_NAME> --query "{Name:name, Location:location, Sku:sku.name}" -o table
...
```powershell
# 同名のリソースが存在するか確認
az aks show -n <AKS_CLUSTER_NAME> -g <RESOURCE_GROUP_NAME> 2>&1
az vm show -n <MONGODB_VM_NAME> -g <RESOURCE_GROUP_NAME> 2>&1
az acr show -n <ACR_NAME> 2>&1
```
...
```powershell
# VM の IP アドレスを取得
$MONGO_IP = az vm list-ip-addresses -g <RESOURCE_GROUP_NAME> -n <MONGODB_VM_NAME> --query "[0].virtualMachine.network.publicIpAddresses[0].ipAddress" -o tsv
