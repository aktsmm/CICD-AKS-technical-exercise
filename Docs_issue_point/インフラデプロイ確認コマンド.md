...
# リソース数を確認
az resource list -g <RESOURCE_GROUP_NAME> --query "length([])"
```
...
```powershell
# AKS クラスターの状態を確認
az aks show -g <RESOURCE_GROUP_NAME> -n aksexercise --query "{Name:name, Status:powerState.code, KubernetesVersion:kubernetesVersion, NodeCount:agentPoolProfiles[0].count}" -o table

# AKS 認証情報を取得
az aks get-credentials -g <RESOURCE_GROUP_NAME> -n aksexercise --overwrite-existing
...
```powershell
# VM の状態を確認
az vm show -g <RESOURCE_GROUP_NAME> -n vm-mongo-dev --query "{Name:name, ProvisioningState:provisioningState, PowerState:instanceView.statuses[1].displayStatus}" -o table

# VM のパブリック IP アドレスを取得
az vm list-ip-addresses -g <RESOURCE_GROUP_NAME> -n vm-mongo-dev --query "[0].virtualMachine.network.publicIpAddresses[0].ipAddress" -o tsv
```
...
# AKS と ACR を統合
az aks update -g <RESOURCE_GROUP_NAME> -n aksexercise --attach-acr acrwizexercise
```
...
az monitor log-analytics workspace show -g <RESOURCE_GROUP_NAME> -n logexercise --query "{Name:name, Location:location}" -o table
```
...
```powershell
# VNet とサブネットを確認
az network vnet show -g <RESOURCE_GROUP_NAME> -n vnetexercise --query "{Name:name, AddressSpace:addressSpace.addressPrefixes[0]}" -o table

az network vnet subnet list -g <RESOURCE_GROUP_NAME> --vnet-name vnetexercise -o table
```
...
```powershell
# Storage Account を確認
az storage account show -n stwizexercise -g <RESOURCE_GROUP_NAME> --query "{Name:name, Location:location, Sku:sku.name}" -o table
...
```powershell
# 同名のリソースが存在するか確認
az aks show -n aksexercise -g <RESOURCE_GROUP_NAME> 2>&1
az vm show -n vm-mongo-dev -g <RESOURCE_GROUP_NAME> 2>&1
...
```powershell
# VM の IP アドレスを取得
$MONGO_IP = az vm list-ip-addresses -g <RESOURCE_GROUP_NAME> -n vm-mongo-dev --query "[0].virtualMachine.network.publicIpAddresses[0].ipAddress" -o tsv
