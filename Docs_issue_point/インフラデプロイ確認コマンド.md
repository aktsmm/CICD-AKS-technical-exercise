...
```powershell
# AKS クラスターの状態を確認
az aks show -g rg-cicd-aks-bbs-01 -n aksexercise --query "{Name:name, Status:powerState.code, KubernetesVersion:kubernetesVersion, NodeCount:agentPoolProfiles[0].count}" -o table

# AKS 認証情報を取得
az aks get-credentials -g rg-cicd-aks-bbs-01 -n aksexercise --overwrite-existing
...
# AKS と ACR を統合
az aks update -g rg-cicd-aks-bbs-01 -n aksexercise --attach-acr acrwizexercise
```
...
az monitor log-analytics workspace show -g rg-cicd-aks-bbs-01 -n logexercise --query "{Name:name, Location:location}" -o table
```
...
```powershell
# VNet とサブネットを確認
az network vnet show -g rg-cicd-aks-bbs-01 -n vnetexercise --query "{Name:name, AddressSpace:addressSpace.addressPrefixes[0]}" -o table

az network vnet subnet list -g rg-cicd-aks-bbs-01 --vnet-name vnetexercise -o table
```
...
```powershell
# 同名のリソースが存在するか確認
az aks show -n aksexercise -g rg-cicd-aks-bbs-01 2>&1
