...
```powershell
az resource list -g <RESOURCE_GROUP_NAME> --query "[].{Name:name, Type:type, State:provisioningState}" -o table
```
...
Name                       Type                                     State
-------------------------  ---------------------------------------  ---------
<ACR_NAME>             Microsoft.ContainerRegistry/registries   Succeeded
<STORAGE_ACCOUNT_NAME>      Microsoft.Storage/storageAccounts        Succeeded
<LOG_ANALYTICS_NAME>                Microsoft.OperationalInsights/workspaces Succeeded
vnetdev               Microsoft.Network/virtualNetworks        Succeeded
<MONGODB_VM_NAME>-nsg           Microsoft.Network/networkSecurityGroups  Succeeded
<MONGODB_VM_NAME>-pip           Microsoft.Network/publicIPAddresses      Succeeded
<MONGODB_VM_NAME>-nic           Microsoft.Network/networkInterfaces      Succeeded
<MONGODB_VM_NAME>               Microsoft.Compute/virtualMachines        Succeeded
<VM_OSDISK_NAME>        Microsoft.Compute/disks                  Succeeded
aks-dev                Microsoft.ContainerService/managedClusters Succeeded
<MONGODB_VM_NAME>/install-mongodb Microsoft.Compute/virtualMachines/extensions Succeeded
...
- Storage Account (`<STORAGE_ACCOUNT_NAME>`) も作成されているが、その後にエラー発生と推定
...
```powershell
az deployment group show -g <RESOURCE_GROUP_NAME> -n mongodb-deployment --query "properties.{State:provisioningState, Duration:duration}" -o json
```
...
| ---------------------------- | ---------------------- | ------------ | -------------------- |
| <ACR_NAME>               | Container Registry     | ✅ Succeeded | アプリデプロイで使用 |
| <STORAGE_ACCOUNT_NAME>        | Storage Account        | ✅ Succeeded | 作成後にエラー?      |
| <LOG_ANALYTICS_NAME>                      | Log Analytics          | ✅ Succeeded | 監視で使用           |
| vnetdev                      | Virtual Network        | ✅ Succeeded | AKS と VM で使用     |
| <MONGODB_VM_NAME>-nsg             | Network Security Group | ✅ Succeeded | MongoDB VM で使用    |
| <MONGODB_VM_NAME>-pip             | Public IP              | ✅ Succeeded | MongoDB VM で使用    |
| <MONGODB_VM_NAME>-nic             | Network Interface      | ✅ Succeeded | MongoDB VM で使用    |
| <MONGODB_VM_NAME>                 | Virtual Machine        | ✅ Succeeded | MongoDB ホスト       |
| <VM_OSDISK_NAME>          | Managed Disk           | ✅ Succeeded | VM の OS ディスク    |
| aks-dev                      | AKS Cluster            | ✅ Succeeded | アプリデプロイ先     |
| <MONGODB_VM_NAME>/install-mongodb | VM Extension           | ✅ Succeeded | MongoDB インストール |
