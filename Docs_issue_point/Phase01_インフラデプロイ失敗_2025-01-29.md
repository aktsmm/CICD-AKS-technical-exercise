...
```powershell
az resource list -g rg-cicd-aks-bbs-01 --query "[].{Name:name, Type:type, State:provisioningState}" -o table
```
...
aksdev                Microsoft.ContainerService/managedClusters Succeeded
...
```powershell
az deployment group show -g rg-cicd-aks-bbs-01 -n mongodb-deployment --query "properties.{State:provisioningState, Duration:duration}" -o json
```
...
| aksdev                       | AKS Cluster            | ✅ Succeeded | アプリデプロイ先     |
