...
```powershell
az resource list -g rg-cicd-bbs --query "[].{Name:name, Type:type, State:provisioningState}" -o table
```
...
```powershell
az deployment group show -g rg-cicd-bbs -n mongodb-deployment --query "properties.{State:provisioningState, Duration:duration}" -o json
```
