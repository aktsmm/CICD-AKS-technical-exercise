...
```powershell
az resource list -g rg-cicd-aks-bbs-001 --query "[].{Name:name, Type:type, State:provisioningState}" -o table
```
...
```powershell
az deployment group show -g rg-cicd-aks-bbs-001 -n mongodb-deployment --query "properties.{State:provisioningState, Duration:duration}" -o json
```
