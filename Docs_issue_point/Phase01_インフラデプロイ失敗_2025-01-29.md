...
```powershell
az resource list -g rg-wiz-exercise-01 --query "[].{Name:name, Type:type, State:provisioningState}" -o table
```
...
```powershell
az deployment group show -g rg-wiz-exercise-01 -n mongodb-deployment --query "properties.{State:provisioningState, Duration:duration}" -o json
```
