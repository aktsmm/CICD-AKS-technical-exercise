...
The resource with name '<ACR_NAME>' could not be found.
```

### 🔍 原因

- ワークフローファイルで ACR 名が `<ACR_NAME>` と指定
- 実際に作成した ACR は `<ACR_NAME>`
- 名前の不一致により認証失敗
...
git commit -m "fix: Update ACR name from <ACR_NAME> to <ACR_NAME>"
...
```bash
az acr build --registry <ACR_NAME> --image guestbook:v3 .
```
...
spec:
  containers:
    - name: guestbook
      image: <ACR_NAME>.azurecr.io/guestbook:v3
```
...
# After
sed -i "s|image: <ACR_NAME>.azurecr.io/guestbook:.*|image: ${{ env.ACR_NAME }}.azurecr.io/${{ env.IMAGE_NAME }}:${{ needs.build-push.outputs.image_tag }}|g" app/k8s/deployment.yaml
```
...
```html
<!-- Before -->
<title>BBS App on Azure Kubernetes Service</title>

<!-- After -->
<title>🚀 Wiz BBS App on Azure Kubernetes Service</title>
```
...
ACRにプッシュ
    ↓
Deployment更新 (image: <ACR_NAME>.azurecr.io/guestbook:f137a12)
    ↓
...
# イメージ詳細
az acr repository show --name <ACR_NAME> --image guestbook:v3
```
