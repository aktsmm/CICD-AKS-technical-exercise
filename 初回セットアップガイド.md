...
# AZURE_CREDENTIALS_ADMINも同じ内容で設定（RBAC設定ワークフロー用）
Get-Content azure_creds.json | gh secret set AZURE_CREDENTIALS_ADMIN -R <YOUR_USERNAME>/<YOUR_REPO>
...
| `AZURE_CREDENTIALS_ADMIN` | `AZURE_CREDENTIALS`と同じ値              | ✅   |
...
> ⚠️ **重要**: `AZURE_CREDENTIALS_ADMIN`は、RBAC ワークフロー（0. Setup RBAC for Service Principal）で使用されます。`AZURE_CREDENTIALS`と同じ内容を設定してください。
...
#### 5-1. （オプション）RBAC 設定ワークフローの実行

> 📝 **注意**: このワークフローは、Service Principal に追加のロール（Resource Policy Contributor 等）を付与します。既にロールが付与されている場合はスキップできます。

```powershell
# Service PrincipalのオブジェクトIDを取得
$objectId = az ad sp show --id <CLIENT_ID> --query id -o tsv

# RBACワークフローを実行（パラメータ指定が必要）
gh workflow run "0. Setup RBAC for Service Principal" \
  -f service_principal_object_id=$objectId \
  -f subscription_id=<YOUR_SUBSCRIPTION_ID>
```

> ⚠️ **重要**: このワークフローは`AZURE_CREDENTIALS_ADMIN`シークレットを使用します。事前に設定されていることを確認してください。
...
#### 4. RBAC ワークフロー実行時の AZURE_CREDENTIALS_ADMIN エラー

**エラー:** `Login failed with Error: Using auth-type: SERVICE_PRINCIPAL. Not all values are present.`

**原因:** `AZURE_CREDENTIALS_ADMIN`シークレットが設定されていない

**解決方法:**

```powershell
# AZURE_CREDENTIALSと同じ内容でAZURE_CREDENTIALS_ADMINを設定
@"
{
  "clientId": "<CLIENT_ID>",
  "clientSecret": "<CLIENT_SECRET>",
...
Get-Content azure_creds.json | gh secret set AZURE_CREDENTIALS_ADMIN -R <owner>/<repo>
Remove-Item azure_creds.json -Force
```
...
AZURE_CREDENTIALS_ADMIN   - AZURE_CREDENTIALSと同じ内容（RBAC用）
AZURE_SUBSCRIPTION_ID     - af0f28a5-faa0-4397-b1ab-ca2006a28e0c
...
IMAGE_NAME                 - bbs-app
AZURE_GITHUB_PRINCIPAL_ID  - 1e07951f-4f0a-4d01-a873-13409d8d298c
AZURE_GRANT_GITHUB_OWNER   - false
```

**ワークフロー実行順序:**

1. （オプション）`0. Setup RBAC for Service Principal` - パラメータ指定が必要
2. `1. Deploy Infrastructure` - インフラデプロイ（15-20 分）
3. AKS と ACR の統合（手動コマンド）
4. `2-1. Build and Deploy Application` - アプリデプロイ（5-10 分）
