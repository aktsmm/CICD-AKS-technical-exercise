...
| **リージョン**            | Japan East                             |
| **リソースグループ**      | `rg-wiz-exercise-a`                    |

### 確認コマンド

```bash
az account show
az group show --name rg-wiz-exercise-a
```
...
# 全リソース一覧
az resource list --resource-group rg-wiz-exercise-a --output table

# AKS詳細
az aks show --resource-group rg-wiz-exercise-a --name aks-wiz-dev

# VM詳細
az vm show --resource-group rg-wiz-exercise-a --name vm-mongo-dev
...
# VM確認
az vm show --resource-group rg-wiz-exercise-a --name vm-mongo-dev
az vm get-instance-view --resource-group rg-wiz-exercise-a --name vm-mongo-dev

# ネットワーク確認
az network nsg rule list --resource-group rg-wiz-exercise-a --nsg-name vm-mongo-dev-nsg
