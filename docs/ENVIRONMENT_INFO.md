...
| **リージョン**            | Japan East                             |
| **リソースグループ**      | `rg-wiz-exercise2`                     |

### 確認コマンド

```bash
az account show
az group show --name rg-wiz-exercise2
```
...
# 全リソース一覧
az resource list --resource-group rg-wiz-exercise2 --output table

# AKS詳細
az aks show --resource-group rg-wiz-exercise2 --name aks-wiz-dev

# VM詳細
az vm show --resource-group rg-wiz-exercise2 --name vm-mongo-dev
...
# VM確認
az vm show --resource-group rg-wiz-exercise2 --name vm-mongo-dev
az vm get-instance-view --resource-group rg-wiz-exercise2 --name vm-mongo-dev

# ネットワーク確認
az network nsg rule list --resource-group rg-wiz-exercise2 --nsg-name vm-mongo-dev-nsg
