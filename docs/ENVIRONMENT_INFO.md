...
```bash
az account show
az group show --name rg-cicd-bbs
```
...
# 全リソース一覧
az resource list --resource-group rg-cicd-bbs --output table

# AKS詳細
az aks show --resource-group rg-cicd-bbs --name aks-wiz-dev

# VM詳細
az vm show --resource-group rg-cicd-bbs --name vm-mongo-dev
...
# VM確認
az vm show --resource-group rg-cicd-bbs --name vm-mongo-dev
az vm get-instance-view --resource-group rg-cicd-bbs --name vm-mongo-dev

# ネットワーク確認
az network nsg rule list --resource-group rg-cicd-bbs --nsg-name vm-mongo-dev-nsg
